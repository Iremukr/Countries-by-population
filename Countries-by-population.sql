...
SELECT name, MAX(percent_one_year_change) AS Max_one_year_change, net_migrants, fertility_rate, median_age FROM countries;

SELECT name, MIN(median_age) AS Min_median_age FROM countries;

SELECT name, ROUND(AVG(population_change)) AS Average_pop_change, median_age, percent_one_year_change, net_migrants, fertility_rate FROM countries
WHERE percent_one_year_change IS NOT 0;

SELECT name, ROUND(AVG(density_per_sq_km))FROM countries;

/* HAVING */
SELECT name, median_age, density_per_sq_km, percent_one_year_change, population_change, percent_of_world_pop, net_migrants, fertility_rate, median_age FROM countries
WHERE percent_of_world_pop >= 0.4 AND population_change >= 40000
GROUP BY median_age
HAVING density_per_sq_km <= 10000 AND density_per_sq_km >= 300
LIMIT 20;

SELECT name, net_migrants FROM countries
WHERE net_migrants > (0.006*population)
ORDER BY population DESC;


SELECT name, MAX(fertility_rate) AS Maximum_fertility FROM countries;

SELECT name, MIN(fertility_rate) AS Minimum_fertility FROM countries;

SELECT name, ROUND(AVG(fertility_rate)) AS Average_fertility FROM countries;

/* Number of counties by fertility */
SELECT COUNT(*),
    CASE
        WHEN fertility_rate >= 4 THEN "High_fertility"
        WHEN fertility_rate <= 2 THEN "Low_fertility"
        ELSE "Average_fertility"
    END as "Fertility_rating"
FROM countries
GROUP BY Fertility_rating;
...
INSERT INTO Target_Countries (name, continent) VALUES ("Ukraine", "Europe");
INSERT INTO Target_Countries (name, continent) VALUES ("Poland", "Europe");
INSERT INTO Target_Countries (name, continent) VALUES ("Portugal", "Europe");
INSERT INTO Target_Countries (name, continent) VALUES ("Lithuania", "Europe");
INSERT INTO Target_Countries (name, continent) VALUES ("United States", "North America");
INSERT INTO Target_Countries (name, continent) VALUES ("Canada", "North America");
INSERT INTO Target_Countries (name, continent) VALUES ("Chile", "South America");
INSERT INTO Target_Countries (name, continent) VALUES ("Brazil", "South America");
INSERT INTO Target_Countries (name, continent) VALUES ("Taiwan","Asia");
INSERT INTO Target_Countries (name, continent) VALUES ("China","Asia");
INSERT INTO Target_Countries (name, continent) VALUES ("Vietnam", "Asia");

/* Countries Querying IN subqueries */
SELECT name, population_change, fertility_rate, median_age
FROM countries WHERE name IN
(SELECT name FROM Target_Countries WHERE continent = "Europe");

/* LIKE */
SELECT name AS Counties_America FROM Target_Countries WHERE continent LIKE "%America%";

/* Countries to live in */
SELECT name, density_per_sq_km, median_age,net_migrants, fertility_rate FROM countries
WHERE fertility_rate > 2 AND median_age >= 30 AND (density_per_sq_km < (0.5*21645) OR net_migrants < 10000)
